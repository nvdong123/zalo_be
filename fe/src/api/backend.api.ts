...
export const authApi = {
  login: (data: LoginRequest) => {
    // Backend sử dụng OAuth2PasswordRequestForm, cần gửi form data
    const formData = new FormData();
    formData.append('username', data.username);
    formData.append('password', data.password);
    
    return request<LoginResponse>('post', '/auth/login', formData);
  },
...
export const bookingRequestApi = {
  ...
  getById: (id: number, tenantId: number) => 
    request<BookingRequest>('get', `/booking-requests/${id}`, { tenant_id: tenantId }),
    
  updateStatus: (id: number, status: string, tenantId: number) =>
    request<BookingRequest>('put', `/booking-requests/${id}`, { status }, { params: { tenant_id: tenantId } }),
    
  delete: (id: number, tenantId: number) => 
    request('delete', `/booking-requests/${id}`, { tenant_id: tenantId })
};
...
export const bookingApi = bookingRequestApi;
