...
const GameManagement: React.FC = () => {
  ...
  const filteredGames = games.filter((game: Game) => {
    const config = game.configurations || {};
    ...
    // Filter by game type
    const matchesType = !filterType || config.game_type === filterType;
    
    // Filter by reward type
    const matchesRewardType = !filterRewardType || config.reward_type === filterRewardType;
    
    // Filter by active status
    const matchesActive = filterActive === undefined || (filterActive === true ? game.status === 'active' : game.status !== 'active');
  ...
  const handleEdit = (game: Game) => {
    ...
    form.setFieldsValue({
      ...game,
      ...config  // Merge configurations into top level for form
    });
    setIsModalVisible(true);
  };
  ...
  const handleSubmit = async (values: any) => {
    // Extract configurations fields
    const { game_type, description, reward_type, reward_value, game_name, status } = values;
    ...
    await gameMutation.mutateAsync(formattedValues);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (game_name: string, record: Game) => {
        const config = record.configurations || {};
        return (
          <div>
    ...
    },
    {
      ...
      key: 'game_type',
      width: 120,
      render: (configurations: any) => {
        const type = configurations?.game_type;
    ...
    },
    {
      ...
      render: (record: Game) => {
        ...
        const rewardInfo = rewardTypeMap[config.reward_type || ''] || { label: config.reward_type || 'N/A', icon: <GiftOutlined /> };
        ...
        return (
          <div>
            ...
            <div style={{ fontSize: 12, color: '#666', marginTop: 2 }}>
              {config.reward_type === 'points' 
                ? `${config.reward_value} điểm`
                : config.reward_type === 'discount'
                ? `${config.reward_value}%`
                : `${config.reward_value?.toLocaleString()}đ`
              }
            </div>
          </div>
        );
        ...
        <div style={{ marginBottom: 16, padding: '12px 16px', backgroundColor: '#f5f5f5', borderRadius: 6 }}>
          <Row gutter={24}>
            ...
            <Col span={6}>
              ...
              <Text>{filteredGames.filter(g => g.status === 'active').length}</Text>
            </Col>
            <Col span={6}>
